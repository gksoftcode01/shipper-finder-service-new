<template>
  <div>
    <h2 id="page-heading" data-cy="TripItemHeading">
      <span v-text="t$('shipperFinderApp.tripItem.home.title')" id="trip-item-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('shipperFinderApp.tripItem.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'TripItemCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('shipperFinderApp.tripItem.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && tripItems && tripItems.length === 0">
      <span v-text="t$('shipperFinderApp.tripItem.home.notFound')"></span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="shipperFinderApp.tripItem.delete.question"
          data-cy="tripItemDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-tripItem-heading" v-text="t$('shipperFinderApp.tripItem.delete.question', { id: removeId })"></p>
      </div>
