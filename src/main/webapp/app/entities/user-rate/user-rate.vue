<template>
  <div>
    <h2 id="page-heading" data-cy="UserRateHeading">
      <span v-text="t$('shipperFinderApp.userRate.home.title')" id="user-rate-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('shipperFinderApp.userRate.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'UserRateCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('shipperFinderApp.userRate.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && userRates && userRates.length === 0">
      <span v-text="t$('shipperFinderApp.userRate.home.notFound')"></span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="shipperFinderApp.userRate.delete.question"
          data-cy="userRateDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-userRate-heading" v-text="t$('shipperFinderApp.userRate.delete.question', { id: removeId })"></p>
      </div>
